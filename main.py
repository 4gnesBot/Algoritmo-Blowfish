...
def hex_to_bin(hexa):
  ...
  for i in range(n):
    ...
    num = hex_to_dec(hexa[i])

    binary4B = dec_to_bin(num)
    binary4B = "0000" + str(binary4B)
    
    binary4B = binary4B[len(binary4B) - 4:]
    binary += binary4B

  return binary

#Binario para hexadecimal
def bin_to_hex(binar):

  num = int(binar, 2)
  hexa = ""
  hexa = dec_to_hex(num)

  while len(hexa) < len(binar)/4:
    hexa = "0" + hexa
...
def xor(a, b):
  ...
  while len(bin_to_hex(a)) > len(ans):
    ans = "0" + ans
...
def addBin(a, b):
  ...
  ans = ""
  n1 = int("0b" + hex_to_bin(a), 2)
  n2 = int("0b" + hex_to_bin(b), 2)

  n1 = (n1 + n2) % modVal

  ans = bin_to_hex(bin(n1))
  ans = "00000000" + ans

  return ans[len(ans) - 8:]

def function_F(pText):
  a = [0, 0, 0, 0]
  ans = ""

  for i in range(4):
    col = int(hex_to_bin(pText[i*2 : i*2+2]), 2)
    a[i] = S[i][col]
...
def round(tempo, pText):
  ...
  left = pText[0:8]
  right = pText[8:16]
  left = xor(left, P[tempo])

  fOut = ""
  fOut = function_F(left)
  ...
  '''print(f"Round {tempo}: {right}{left}")'''
...
def text_to_ascii(origText):
  tempAscii = ""
  
  for i in range(len(origText)):
    t = str(ord(origText[i]))
    while len(t) < 3:
      t = "0" + t
    tempAscii += t

  while len(tempAscii)%18 != 0:
    tempAscii += "032"

  return tempAscii

def format_hex(ascii):
  tempHexa = ""

  tempHexa = dec_to_hex(int(ascii))

  while len(tempHexa)%16 != 0:
    tempHexa = "0" + tempHexa

  return tempHexa

def hex_to_text(cipherText):
  tempHexa = cipherText
  tempH = []

  for i in range(int(len(tempHexa)/16)):
    tempH.append(tempHexa[i*16 : i*16 + 16])

  tempA = []

  for i in range(int(len(tempH))):
    tempA.append(hex_to_dec(tempH[i]))
    while len(tempA[i])%3 != 0:
      tempA[i] = "0" + tempA[i]

  tempText = ""

  for i in range(int(len(tempA))):
    for j in range(int(len(tempA[i])/3)):
      tempText += chr(int(tempA[i][j*3 : j*3 + 3]))
...
while True:
  ...
  origText = (str(input("Digite o texto: ")))
  origKey = (str(input("Digite a chave: ")))
  mode = (str(input("Digite C para criptografar ou D para descriptografar: ")))

  if origKey == "":
    origKey = " "

  key = dec_to_hex(int(text_to_ascii(origKey)))
  ...
  if mode == "c" or mode == "C":
    ...
    while len(origText)%6 != 0:
      origText += " "

    for i in range(int(len(text_to_ascii(origText))/18)):
      tempT.append(text_to_ascii(origText)[i*18 : i*18 + 18])

    for i in range(int(len(tempT))):
      plainText = format_hex(tempT[i])
  ...
  if mode == "d" or mode == "D":
    ...
    for i in range(int(len(origText)/16)):
      plainText = origText[i*16 : i*16 + 16]
      cipherText += decrypt(plainText)

    finalText = hex_to_text(cipherText)
